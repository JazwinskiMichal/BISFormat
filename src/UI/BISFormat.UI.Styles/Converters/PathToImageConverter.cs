...
{
    public class PathToImageConverter : DependencyObject, IValueConverter
    {
        ...
        public Brush Color { get; set; } = new SolidColorBrush(System.Windows.Media.Color.FromRgb(67, 67, 67));

        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            Path path = new() { Fill = Color, Stroke = new SolidColorBrush(Colors.Transparent), StrokeThickness = 0 };
            path.Data = Geometry.Parse(value != null && !string.IsNullOrEmpty(value.ToString()) ? value.ToString() : IconIfNull);
